      ...
      <>
        <Link href={`/posts/${hit.objectID}`}>
          <a className={clsx(cardStyle)} onClick={() => closeModal()}>
            <h2 className="text-lg font-semibold text-ngray-900 dark:text-ngray-100 mb-3 truncate">
              {hit.title}
            </h2>
            <p className="text-ngray-900 dark:text-ngray-100 truncate pb-0.5">
              <Snippet
                ...
                classNames={{
                  highlighted:
                    'border-b-2 border-ppink-200 bg-transparent text-inherit',
                }}
              />
            </p>
          </a>
    ...
    <InstantSearch indexName="blog_datas" searchClient={searchClient}>
      ...
      <Pagination
        classNames={{
          ...
          disabledItem:
            'text-ngray-300 dark:text-ngray-600 hover:bg-inherit dark:hover:bg-inherit',
